...
exports.delete_a_trip = function (req, res) {
    ...
    Trip.findOne({ ticker: req.params.tripTicker }, function (err, trip) {
        if (!trip) {
            res.status(404).json({ message: 'Trip does not exists' });
        } else {
            Trip.deleteOne({ ticker: req.params.tripTicker }, function (err, delRes) {
                if (err) {
                    res.send(err);
                }
                else {
...
exports.update_a_trip = function update_a_trip(req, res) {
    ...
    Trip.findOne({ ticker: req.params.tripTicker }, function (err, trip) {
        ...
        Trip.findOneAndUpdate({ ticker: req.params.tripTicker }, tripUpdated, { new: true, runValidators: true, context: 'query' }, function (err, trip) {
            if (err) {
                if (err.name == 'ValidationError') {
                    res.status(422).send(err);
                }
